...
At most one stage per event can be `active`, enforced by the `uqOneActiveStagePerEvent` unique index on `ticket_stages(eventId, activeFlag)` (`activeFlag` is a generated column, `1` only when `status = 'active'`, `NULL` otherwise — NULLs don't collide in a unique index, so any number of non-active stages coexist fine). Any write that promotes a stage to `active` must first demote whatever else is currently `active` for that event to `closed` in the same transaction, or MySQL rejects it with `ER_DUP_ENTRY`. Current call sites: `updateEvent`'s stage-reconciliation loop (`server/controllers/events.controllers.js`), `activateDueStages` (`server/jobs/scheduler.js`).
